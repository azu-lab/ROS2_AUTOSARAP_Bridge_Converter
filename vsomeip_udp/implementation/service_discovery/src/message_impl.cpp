...
#define VSOMEIP_EXPLICIT_FLAG 0x20

bool message_impl::get_explicit_flag() const {
    return ((flags_ & VSOMEIP_EXPLICIT_FLAG) != 0);
}

void message_impl::set_explicit_flag(bool _is_set) {
    if (_is_set)
        flags_ |= flags_t(VSOMEIP_EXPLICIT_FLAG);
    else
        flags_ &= flags_t(~VSOMEIP_EXPLICIT_FLAG);
}
